# 1758번 알바생 강호
# 문제) 스타박스는 손님을 입장시킬 때 독특한 방법으로 입장시킨다.
...
# 입력) 첫째 줄에 스타박스 앞에 서 있는 사람의 수 N이 주어진다. N은 100,000보다 작거나 같은 자연수이다. 둘째 줄부터 총 N개의 줄에 각 사람이 주려고 하는 팁이 주어진다. 팁은 100,000보다 작거나 같은 자연수이다.
# 출력) 강호가 받을 수 있는 팁의 최댓값을 출력한다.
N = int(input())
a_list = []
for i in range(N):
    a = int(input())
