...
salList = list(map(int,input().split()))

sumSal = 0
sumSalList = [0]
for i in salList:
    sumSal+=i
    sumSalList.append(sumSal)

sumValueList =[]
for i in range(0,len(sumSalList)-M):
    sumValue = sumSalList[i+M]-sumSalList[i]
    sumValueList.append(sumValue)
print(max(sumValueList))

# *****************************
# 백준 9375번 / 실버3 / 패션왕 신해빈
# *****************************
...
for i in range(T):
    ...
    dict = {}
    for i in range(n):
        cloList = list(map(str,input().split()))
        if(cloList[1] in  dict):
            dict[cloList[1]]+=1
        else:
            dict[cloList[1]]=1
