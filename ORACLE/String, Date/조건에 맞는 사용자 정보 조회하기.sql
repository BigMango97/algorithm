...
SELECT UGB.WRITER_ID, UGU.NICKNAME, CITY||' '||STREET_ADDRESS1||' '||STREET_ADDRESS2 AS 전체주소, REGEXP_REPLACE(UGU.TLNO,'(.{3})(.+)(.{4})','\1-\2-\3') AS 전화번호
FROM USED_GOODS_BOARD UGB, USED_GOODS_USER UGU 
WHERE UGB.WRITER_ID = UGU.USER_ID
HAVING COUNT(UGB.WRITER_ID)>=3
GROUP BY UGB.WRITER_ID,UGU.NICKNAME, UGU.CITY, UGU.STREET_ADDRESS1, UGU.STREET_ADDRESS2, UGU.TLNO
